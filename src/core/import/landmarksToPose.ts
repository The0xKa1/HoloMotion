...
import type { MotionVector3 } from "../three-compat.js";
import type { JointPose, QuaternionTuple, SkeletonPose, Vec3Meters } from "../../types/motion.js";
...
export function landmarksToPose(landmarks: NormalizedLandmark[]): SkeletonPose | null {
...
function joint(position: Vec3Meters): JointPose {
  return { position, rotation: IDENTITY };
}

function vec(p: MotionVector3): Vec3Meters {
  return [p.x, p.y, p.z];
}

function midpoint(a: MotionVector3, b: MotionVector3): Vec3Meters {
  return [(a.x + b.x) * 0.5, (a.y + b.y) * 0.5, (a.z + b.z) * 0.5];
}

function lerp3(a: Vec3Meters, b: Vec3Meters, t: number): Vec3Meters {
  return [
    a[0] + (b[0] - a[0]) * t,
    a[1] + (b[1] - a[1]) * t,
    a[2] + (b[2] - a[2]) * t,
