...
import type { JointName, SkeletonPose, Vec3Meters } from "../../types/motion.js";

const TARGET_HEIGHT_METERS = 1.0;

export function postProcessFrames(input: Array<SkeletonPose | null>, fps: number): SkeletonPose[] {
...
function fillGaps(input: Array<SkeletonPose | null>): SkeletonPose[] {
  ...
  const result: SkeletonPose[] = new Array(total);
...
function smoothOverTime(frames: SkeletonPose[], fps: number): SkeletonPose[] {
...
function recenter(frames: SkeletonPose[]): SkeletonPose[] {
  return frames.map((frame) => {
    const [px, py, pz] = frame.pelvis.position;
    const out = {} as SkeletonPose;
    JOINT_NAMES.forEach((name: JointName) => {
      const src = frame[name];
      out[name] = {
...
function normalizeHeight(frames: SkeletonPose[]): SkeletonPose[] {
...
function clonePose(pose: SkeletonPose): SkeletonPose {
  const out = {} as SkeletonPose;
  JOINT_NAMES.forEach((name: JointName) => {
    const src = pose[name];
    out[name] = {
