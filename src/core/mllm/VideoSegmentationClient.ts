...
export interface MllmVideoSegment {
  id: string;
  name: string;
  actionLabel: string;
  startSec: number;
  endSec: number;
  confidence: number;
  metadata: MllmSegmentMetadata;
  notes: string;
}

export interface MllmVideoSegmentationResult {
...
export function normalizeSegmentationContent(content: string, durationSeconds: number): MllmVideoSegmentationResult {
  ...
  const segments: MllmVideoSegment[] = [];
  const dropped: Array<{ index: number; reason: string; raw: unknown }> = [];
...
type SegmentAttempt = { segment: MllmVideoSegment } | { reason: string };

function tryNormalizeSegment(value: unknown, index: number, durationSeconds: number): SegmentAttempt {
...
function normalizeMetadata(value: unknown): MllmSegmentMetadata {
  if (!isRecord(value)) return {};
  const out: MllmSegmentMetadata = {};
