...
import { UserProfileStore, type UserProfile } from "./UserProfile.js";
...
export type CalibrationStatus =
  ...
  | { phase: "sampling"; progress: number }
  | { phase: "done"; profile: UserProfile }
  | { phase: "failed"; reason: string };

export class CalibrationController {
  private userPose: UserPoseSource;
  private store: UserProfileStore;
  private listeners: Array<(status: CalibrationStatus) => void> = [];
  private status: CalibrationStatus = { phase: "idle" };
  private tickHandle: number | null = null;
  private samples: number[][] = [];
  private startedAt = 0;

  constructor(userPose: UserPoseSource, store: UserProfileStore) {
  ...
  onChange(listener: (status: CalibrationStatus) => void): () => void {
  ...
  private tick(): void {
    ...
    const flat: number[] = new Array(fresh.length * 3);
    for (let i = 0; i < fresh.length; i += 1) {
      const lm = fresh[i] as NormalizedLandmark;
      flat[i * 3] = lm.x;
      flat[i * 3 + 1] = lm.y;
      flat[i * 3 + 2] = lm.z;
    }
  ...
  private emit(status: CalibrationStatus): void {
    this.status = status;
    this.listeners.forEach((fn) => fn(status));
  }
}

function averageSamples(samples: number[][], count: number): NormalizedLandmark[] {
  const out: NormalizedLandmark[] = new Array(count);
...
function stabilityCheck(samples: number[][], count: number): number {
...
function derive(averaged: NormalizedLandmark[]): UserProfile | null {
