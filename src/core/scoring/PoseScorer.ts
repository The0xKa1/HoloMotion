...
import { SCORING_BONES, type MpAnchor } from "./boneTable.js";
...
import type { UserPoseSource } from "./UserPoseSource.js";
...
export function applyLiveScore(packet: FrameStreamPacket, ctx: ScorerContext): void {
  ...
  const candidates: SkeletonPose[] = history.length > 0 ? history : [frame.seedJoints];

  let best: BucketSnapshot | null = null;
  ...
  let weightSum = 0;
  let scoreSum = 0;
  let worst: JointMetric | null = null;
...
function countBadBones(vecs: MotionVector3[], cal: UserProfileBoneLengths): number {
  const checks: Array<[MotionVector3 | undefined, MotionVector3 | undefined, number]> = [
...
type UserProfileBoneLengths = NonNullable<ReturnType<UserProfileStore["get"]>>["boneLengths"];

function cloneVecs(src: MotionVector3[]): MotionVector3[] {
  return src.map((v) => new THREE.Vector3(v.x, v.y, v.z));
}

function pushBucket(buckets: Buckets, id: string, value: number): void {
  const bucket = buckets[id] ?? (buckets[id] = { sum: 0, n: 0 });
  bucket.sum += value;
  bucket.n += 1;
}

function yawAlignToCoach(userVecs: MotionVector3[], seed: SkeletonPose): void {
...
function userBoneVec(
  vecs: MotionVector3[],
  midHips: MotionVector3,
  midShoulders: MotionVector3,
  a: MpAnchor,
  b: MpAnchor,
): MotionVector3 | null {
...
function coachBoneVec(seed: SkeletonPose, from: JointName, to: JointName): MotionVector3 | null {
...
function jointAngleUser(vecs: MotionVector3[], a: number, b: number, c: number): number | null {
  const va = vecs[a];
  const vb = vecs[b];
  const vc = vecs[c];
...
function jointAngleCoach(seed: SkeletonPose, a: JointName, b: JointName, c: JointName): number | null {
...
function angleAt(
  ...
  const ba: [number, number, number] = [a[0] - b[0], a[1] - b[1], a[2] - b[2]];
...
function midpoint(a: MotionVector3 | undefined, b: MotionVector3 | undefined): MotionVector3 | null {
  if (!a || !b) return null;
