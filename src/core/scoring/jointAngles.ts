...
export interface JointAngleSpec {
  metricId: string;
  // Coach side joints, by JointName.
  coach: { a: JointName; b: JointName; c: JointName };
  // MediaPipe POSE 33-index triplet for the same joint.
  user: { a: number; b: number; c: number };
}

export const JOINT_ANGLES: JointAngleSpec[] = [
  // Knee: hip → knee → ankle
