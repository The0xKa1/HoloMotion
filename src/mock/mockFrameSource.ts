...
const identityQuat: QuaternionTuple = [0, 0, 0, 1];
const jointOrder: JointName[] = [
  "pelvis",
  "spine",
  "chest",
  "neck",
...
function metricRisk(score: number): MetricRisk {
  if (score < 68) return "risk";
  if (score < 82) return "warn";
  return "good";
}

function buildMetrics(options: MockFrameOptions): JointMetric[] {
...
function joint(position: [number, number, number], rotation: QuaternionTuple = identityQuat) {
  return {
...
function buildPose(options: MockFrameOptions, variant: "seed" | "user"): SkeletonPose {
  const t = options.progress * Math.PI * 2;
...
const FALLBACK_METRIC: JointMetric = {
  id: "pelvis",
  name: "pelvis",
  base: 100,
  variance: 0,
...
export function createMockPacket(options: MockFrameOptions): FrameStreamPacket {
  ...
  const worst: JointMetric = sorted[0] ?? FALLBACK_METRIC;
